...
if [[ -z "$NF_MDP_MSBUILDTASK_PATH_EFFECTIVE" ]]; then
  ...
  if [[ -n "$MONO_SYSTEM_DRAWING" ]]; then
    NF_MDP_TEMP_DIR="$(mktemp -d /tmp/nf-mdp-w5500-XXXXXX)"
...
echo "[1/2] Restoring packages for W5500 bring-up test..."
...
required_refs=(
  ...
  "$ROOT_DIR/packages/nanoFramework.Runtime.Events.1.11.32/lib/nanoFramework.Runtime.Events.dll"
)
...
if [[ "$missing_refs" -eq 1 ]]; then
  echo "[info] Restored packages incomplete; installing W5500 bring-up dependencies..."
  ...
  /usr/bin/nuget install nanoFramework.Runtime.Events -Version 1.11.32 -OutputDirectory "$ROOT_DIR/packages"
fi

echo "[2/2] Building W5500 bring-up test ($CONFIGURATION)..."
...
echo "W5500 bring-up test build succeeded."
